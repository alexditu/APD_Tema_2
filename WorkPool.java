...
public class WorkPool {
	...
	public WorkPool(int nThreads) {
		...
		crtFragProcessed = new String[nThreads];
	...
	public synchronized PartialSolution getWork(String filename) {
		if (tasks.size() == 0) { // workpool gol
			nWaiting++;
			/* condtitie de terminare:
			 * nu mai exista nici un task in workpool si nici un worker nu e activ 
		...
		for (int i = 0; i < tasks.size(); i++) {
			if ( ((ReducePartialSolution)tasks.get(i)).getFileName().equals(filename)) {
				return tasks.remove(i);
			}
		}
		
		// aici nu ar trebui sa se ajunga
		return tasks.remove();
	...
	synchronized void setCrtFragProcessed(String filename, int threadNum){
		crtFragProcessed[threadNum] = filename;
	}
	
	synchronized boolean isFileProcessed(String filename) {
		for (int i = 0; i < nThreads; i++) {
			if (crtFragProcessed[i].equals(filename)) {
				return true;
			}
		}
		return false;
